...
staffNumber = Mobile.getText(findTestObject('MayaHome/mayahome - MobileSpy 1/propertyagent - Txtfield'), 0, FailureHandling.OPTIONAL).length()

if (staffNumber == 8) {
    for (int i = 1; i < 2; i++) {
        switch (i) {
            case '1':
                Mobile.setText(findTestObject('MayaHome/mayahome - MobileSpy 1/propertyagent - Txtfield'), 'syafiqhaziq', 
                    0)

                Mobile.tap(findTestObject('MayaHome/mayahome - MobileSpy 5/confirm - Btn'), 0)

                Mobile.verifyElementVisible(findTestObject('MayaHome/mayahome - MobileSpy 1/staffNumberError - Txt'), 0, 
                    FailureHandling.CONTINUE_ON_FAILURE)
            case '2':
                Mobile.setText(findTestObject('MayaHome/mayahome - MobileSpy 1/propertyagent - Txtfield'), '!@#$%^&*', 0)
